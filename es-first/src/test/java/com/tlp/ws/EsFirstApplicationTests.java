...
public class EsFirstApplicationTests {
    ...
    //使用queryStringQuery完成单字符串全文查询
        ...
        SearchQuery searchQuery2 = new NativeSearchQueryBuilder().withQuery(termQuery("title", "基础的")).build();
        ...
        SearchQuery searchQuery5 = new NativeSearchQueryBuilder().withQuery(boolQuery().should(rangeQuery("status").lt(1)).must(matchQuery("realName", ""))).withSort(sortBuilderName).withSort(sortBuilderId).build();
    ...
    public void add() {
        Blog blog = new Blog();
        blog.setId(201904161554L);
        blog.setContent("C语言是一门面向过程、抽象化的通用程序设计语言，广泛应用于底层开发。C语言能以简易的方式编译、处理低级存储器。C语言是仅产生少量的机器语言以及不需要任何运行环境支持便能运行的高效率程序设计语言。尽管C语言提供了许多低级处理的功能，但仍然保持着跨平台的特性，以一个标准规格写出的C语言程序可在包括一些类似嵌入式处理器以及超级计算机等作业平台的许多计算机平台上进行编译");
        blog.setPostdate(new Date());
        blog.setTitle("C语言基础知识学习");
        blog.setUrl("www.clang.com");
    ...
    public void find1() {
       ...
       SearchQuery searchQuery1 = new NativeSearchQueryBuilder().withQuery(matchQuery("address","美国")).build();
        ...
        SearchQuery searchQuery8 = new NativeSearchQueryBuilder().withQuery( boolQuery().must(matchQuery("address","美国")).mustNot(matchQuery("age",30))).build();

        SearchQuery searchQuery9 = new NativeSearchQueryBuilder().withQuery( boolQuery().filter(boolQuery().should(matchQuery("address","美国")).should(matchQuery("name","姚明")))).build();
